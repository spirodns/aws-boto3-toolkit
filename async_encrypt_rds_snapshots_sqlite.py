...
async def main():
    ...
    # snapshots = rds.describe_db_snapshots(Filters=[{'Name': 'encrypted', 'Values': ['false']}])['DBSnapshots']
    snapshots = list_rds_db_snapshots()

    # tasks = [encrypt_snapshot(snapshot['DBSnapshotIdentifier']) for snapshot in snapshots]
